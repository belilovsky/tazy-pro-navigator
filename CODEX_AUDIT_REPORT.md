...
- `index.html`:
  ...
  - блоки `Завод` и `Инженерия` переведены на `factory-cutaway.png` и `engineering-plan.png` из `assets/generated/`.
- `src/app.js`:
  - референс цепочки переведён на `assets/generated/production-chain.png`.
...
### Известные ограничения
- Текущий пакет закрывает ключевые визуальные поверхности сайта, но не является финальной бренд-библиотекой. Для следующего прохода остаётся расширение image pack под document room, экспортные материалы и дополнительные секции.
